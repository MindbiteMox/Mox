...
htmx.on('htmx:configRequest', function (evt) {
    if (evt.detail.triggeringEvent && evt.detail.triggeringEvent.type === 'click' && evt.detail.triggeringEvent.target.tagName === 'BUTTON' && evt.detail.triggeringEvent.target.name) {
        evt.detail.parameters[evt.detail.triggeringEvent.target.name] = evt.detail.triggeringEvent.target.value;
    }
});

htmx.defineExtension('mox-prefix', {
    onEvent: function (name, evt) {
        if (name === 'htmx:configRequest') {
            var prefixElement = htmx.closest(evt.detail.elt, "[prefix]");
            if (prefixElement) {

                var prefix = prefixElement.getAttribute('prefix');

                for (var i in evt.detail.parameters) {
                    if (!i.startsWith(prefix)) {
                        delete evt.detail.parameters[i];
                    }
                }

                for (var i in evt.detail.unfilteredParameters) {
                    if (!i.startsWith(prefix)) {
                        delete evt.detail.unfilteredParameters[i];
                    }
                }
            }
        }
